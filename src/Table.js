...
function GlobalFilter({ preGlobalFilteredRows, globalFilter, setGlobalFilter }) {
  ...
  return (
    <label className='flex gap-x-2 items-baseline w-full'>
      ...
      <input
        type='text'
        className='rounded-md w-full h-8 border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50'
        value={value || ""}
        onChange={(e) => {
  ...
  return (
    <span
      className={classNames(
        ...
        status.startsWith("alive") ? "bg-green-100 text-green-800" : null,
        status.startsWith("dead") ? "bg-yellow-100 text-yellow-800" : null,
        status.startsWith("unknown") ? "bg-red-100 text-red-800" : null
      )}
    >
      {status}
    </span>
